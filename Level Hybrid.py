...
def cropStraightImage(img):
    ...
    for part in partList:
        ...
        img = cv.putText(img, part.description,(cropping_cx_straight[part.position_x-1]+20 ,cropping_cy_straight[part.position_y-1]+20 ),cv.FONT_HERSHEY_SIMPLEX ,0.3,(0,0,255),1,cv.LINE_AA)    #Annotate and save image    
    img_name = "./group5_demo_images/lvl3_tray_{}_annotated.png".format(tray_counter)
...
def cropCurvedImage(img,isLeft):
    ...
    for part in partList:
        ...
        img = cv.putText(img, part.description,(cropping_cx_straight[part.position_x-1] ,cropping_cy_straight[part.position_y-1] ),cv.FONT_HERSHEY_SIMPLEX ,0.3,(0,0,255),1,cv.LINE_AA)
...
def readContours(isStraight):
    ...
    if isStraight:
        ...
        imgDefect1 = cv.imread("./shapes/Shapes_Moving/straight_defect_1.jpeg")
        imgDefect2 = cv.imread("./shapes/Shapes_Moving/straight_defect_2.jpeg")
        imgDefect3 = cv.imread("./shapes/Shapes_Moving/straight_defect_3.jpeg")
        straightContour = getShape(imgStraight)
        defect1Contour =  getShape(imgDefect1)
        defect2Contour = getShape(imgDefect2)
        defect3Contour = getShape(imgDefect3)
        ...
        imgStraight = cv.imread("./shapes/Shapes_Moving/straight_shape.jpeg")
        imgDefect1 = cv.imread("./shapes/Shapes_Moving/curved_defect_1.jpeg")
        imgDefect2 = cv.imread("./shapes/Shapes_Moving/curved_defect_2.jpeg")
        imgDefect3 = cv.imread("./shapes/Shapes_Moving/curved_defect_3.jpeg")
        imgDefect3 = cv.flip(imgDefect3,0)
    ...
    for i,contour in enumerate(contours):
        ...
        if hierarchy[0][i][3]==-1:
            ...
            if cv.contourArea(contour) > 1500 and cv.contourArea(contour)<15000:
                ...
                contour_filter.append(contour)
    """
    We could have an issue here with the opening and closing of the window 
    """
    plt.figure(figsize = (ds,ds))
